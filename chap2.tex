...
Antibodies have an important role in adaptive immunity by specifically binding to invading pathogens like bacteria and viruses.
...
However all these factors leads back to antigen binding measured by nearby T cells and effecting the mutual probabilities of proliferation or apoptosis \cite{Bannard_Cyster_2017}, \cite{victora2012germinal}.
As an example in a GC there is a direct selection for higher BCR affinity to sequester more antigen, while the selection against binding self antigens is mediated by the binding competition that leads to less binding of less antigen from the pathogen.
Therefore, in its most simple way, affinity maturation is driven by affinity towards the presented antigen and all the effects also attributed to selection, like folding, stability and aggregation, is selected by their effect on affinity.

Affinity maturation is clearly a strong selection imposed on the B cell evolution and this poses a challenge to phylogenetic reconstruction.% which presumably will break .
To validate the correctness of phylogenetic methods simulation studies are the most important tool.
...
But while many groups have simulated affinity maturation over time \cite{Reshetova_2017}, \cite{Balelli_2016}, \cite{Childs_Baskerville_Cobey_2015} there have been no example of affinity centric simulations on single sequence level.

We propose a simple model of affinity maturation that models sequence fitness as a function of BCR affinity.
If the BCR affinity is high it means that the B cell will endocytose a lot of antigen and thereby get plenty of T cell help decreasing the changes of undergoing apoptosis while increasing the chances of proliferation.
...
Under this setup it is not the absolute affinity that matters but rather the affinity relative to the affinities of all competing cell in the whole GC, and the total amount of antigens to compete for.
...
The purpose of the presented simulation method is to generate more realistic sequence simulations that can be widely used to assess performance of phylogenetic methods.
The model is suppose to capture the most dominant effect of affinity maturation without being a full mechanistic model, however sufficient to recapitulate the features of real GC data.
...
A neutral branching process, independent from the later described affinity model, was setup as a reference point.
This can be viewed as a model of cell divisions where at each generation a cell can either die or produce a number of offspring and each offspring has some probability of carrying a mutation.
...
At initialization of this simulation the root sequence is given and this gets evolved through a stochastic branching process that also includes stochastic introduction of mutations at each generation.
The branching process is controlled by an arbitrary discrete distribution, but due to the flexible nature and convenient mathematical properties of the Poisson distribution, this is the default choice.
Therefore in future sections the branching process is controlled by a $\operatorname{Pois}(\lambda)$ progeny distribution.
Furthermore at each generation all progeny cells will undergo a mutation process determined by another discrete distribution, again we use Poisson.
...
Since the mutations happen sequentially there is a possibility of back mutations.
Mutations can be introduced using a uniform probability over all sites, but because it is well known that BCR sequences do not mutate uniformly random \cite{Yeap2015-nl} we used an empirical approximation of the neutral mutation process called S5F \cite{cui2016model}.
The S5F model contain mutability of the middle base of all 5'mer motifs, and substitution preferences given a mutation i.e. if a random process chooses to mutate the 5'mer \texttt{AAAAA}, then the S5F model also provides a list of probabilities for each middle base substitution, either $P(\AAAAA \rightarrow \AATAA)$, $P(\AAAAA \rightarrow \AAGAA)$ or $P(\AAAAA \rightarrow \AACAA)$, summing to 1.
A 5'mer mutability cannot be used directly on a site at the start or end of a sequence because of missing context, so in those cases we put the an unknown base, N, instead of the missing context and marginalized over all possible motifs fitting this ambiguous context.

Termination of the neutral branching process was enforced in either of three ways: 1) by simulating under a subcritical process ($\lambda < 1$) \cite{gwp} following it until extinction, 2) by using a stopping time $T$, or 3) by stopping after a max population of $N$ has been reach.
In addition we introduced a parameter for down-sampling the cell population down to $n$ cells after termination.
Down-sampling allow us to simulate incomplete sampling of cells e.g. by fall out during PCR, sequencing or quality control steps.
This gives a five parameter model, tabulated in table \ref{neut_constants}, but only one of the stopping criteria can be used, effectively making it a four parameter model.
...
$\lambda_0$ & $\operatorname{Pois}(\lambda_0)$ sequence mutability \\
$T$ & Stopping time \\
$N$ & Stopping at this number of sequences \\
$n$ & Down-sampled number of sequence
\end{tabular}
\caption{
...
\subsection{Simulating with affinity}

\subsubsection{Model concept and biological assumptions}
In the following sections we will be describing the model for affinity selection in detail but lets first make some basic assumptions to keep later definitions simpler.
First of all the system we intent to model is affinity maturation that happening in the GC reaction.
We argue that the major effect in this reaction is affinity towards a target antigen and therefore other effects are ignored.
