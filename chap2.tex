...
%EM This is a sentence fragment.
...
Y(0) = 0, \ \ \ Y \left(\frac{f_{\full}}{U} \right) = 1, \ \ \ Y(f_{\full}) = 2 - \epsilon
  \label{eq:BA_cond}
\end{equation}
%EM I don't really know how to interpret "close to fully activated" besides that it gives 2-epsilon. Do we actually need f_full? Most places I only see f_full/U, or its inverse.
Here $0 < f_{\full} \leq 1$ is the fraction of bound BCRs ($B^i_{\bound}$) that is sufficient to make the B cell close to fully activated, where close is in the sense that $Y(f_{\full})$ is close to the maximum $\lambda$ of 2.
Therefore binding more antigen does not result in much change in the progeny distribution.
...
%EM alpha is the lower bound? It looks like the lower bound is 0.
...
%EM I find the following sentence confusing. Is it necessary?
...
%EM Here we have f_full by itself but it would seem as good to just say that it has a lot of bound antigen and this can lead to a clonal burst.
...
$$
T_{\burst} = \log_\kappa \left(\text{carrying capacity} \right) = \log_\kappa\left( U \frac{[A_{\total}] - [A]}{f_{\full}} \right)
$$
%EM kappa was K above, but in any case I suggest just using 2 in the equation above.
...
%EM ...? Doesn't the statement below leading to the Pois(1) depend on U?
The choice of $f_{\full} = 1$ has the interpretation that when half of the BCRs on a B cell are binding antigen it has a $\operatorname{Pois}(1)$ progeny distribution and when all BCRs are binding antigen, the progeny distribution increases to $\operatorname{Pois}(2-\epsilon)$.
\begin{figure}[!ht]
\begin{center}
