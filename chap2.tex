...
%EM Whoa, really? Mutability is lambda_0, while the progeny distribution parameters are lambda_i? That's a little confusing. Can we use \mu or something instead? Also, it would be great to get a little more detail on how these mutations are introduced WRT S5F. Do we draw from the poisson each step and then use S5F to decide where those mutations are?
...
Here $\epsilon$ can be regarded as a small value (e.g. $10^{-3}$), only added as a necessity to solve the unknown variables.
...
%EM I'd suggest moving these next three paragraphs to the beginning of this subsection. Easiest to start with the more concretely set things.
...
%EM Isn't this the inverse fraction bound?
...
%EM You do updating every generation then, I think? Please mention this.
...
%EM isn't the number of targets t_n?
...
%EM Yes, I agree with this non-linearity, but the title of this section is about multiple targets...
...
%EM before this figure can you define what target overlap is?
...
\vspace{-6mm} \hspace{26mm} (a) \hspace{39mm} (b) \hspace{39mm} (c)
    \caption{
    \label{fig:epistasis}
    %EM I can see why you are connecting multiple targets and epistasis here, and it definitely leads to epistasis. But I think it would be clearest to start by talking about multiple targets and then say that this leads to an epistatic effect.
    %EM BTW this is a gorgeous figure that perfectly shows what you intend.
        ...
        In a) no overlap makes a long distance between the two peaks in fitness, in b) peaks are getting closer when targets overlap, and in c) when the overlap is complete the two targets match and the system no longer is epistatic. %EM ... except for the nonlinear activation function.
        }
\end{figure}
