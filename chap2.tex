...
%EM Of course I'd like \operatorname{mut}! (You can ignore me for your thesis, but not for the eventual write-up. :) )
...
In the situation where a newly arising mutant has higher affinity than the rest of the population the fraction of BCRs binding antigen on this mutant will approach the fitness limit ($f_{\full} - \epsilon$), resulting in a high probability that the clone will expand rapidly to overtake the GC population, also known as a clonal burst.
...
%EM We can have non-additive but non-multimodal fitness landscapes, so they aren't quite equivalent.
Multimodality in the fitness landscape follows from epistasis, which is defined as non additive interaction between mutations, and is widely observed in nature e.g.\ the evolution of influenza nucleoprotein \cite{gong2013stability}.
