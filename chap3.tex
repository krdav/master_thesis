...
% @matsen I just married you two words ;) When talking about observations along a process it is referred to as the history.
One interpretation of the COAR value is that it is the distance between the true and inferred mutation histories as shown in figure \ref{fig:mutation_process}.
...
% @matsen It could, surely. Don't know exactly why we didn't... Something you think I should include?
...
This script is available in the codebase of \texttt{GCtree} (\url{github.com/matsengrp/gctree}).
...
In a worst-case scenario we found an expected one nucleotide error per reconstructed lineage sequence, based on affinity simulation with 4-8 sequences (including the a priori know root and leaf) in a lineage, see b) in figure \ref{fig:Laura-affsim_runstat} appendix B.

We use two kinds of simulation, with and without selection.
...
What we do find surprising is that all inference methods seems to be equally effected by the imposed affinity selection.
...
Looking at typical trees simulated from both the neutral and affinity selected process (see trees in appendix B) it looks like trees are relatively shallow i.e.\ the number of internal nodes from the leaf to the root is small.
...
This trait could explain why is there is no apparent benefit of integrating AID motifs into an inference algorithm like IgPhyML, the effect could in fact be positive but too small to be detected in our ASR validation using shallow trees.
