...
Using the assumption of independence between sites ($p_i$) the parsimony score of a tree ($T$) can be calculated as a sum of all the scores at each site: $S(T) = \sum_{i=1}^m F(p_i | T)$.
...
The branch and bound algorithm can is a way of decreasing search space but still finding the best solution, however even with this hill climbing methods have to be used.
...
It it clear that the MP method for tree inference is very simple and while this will work in some applications the criterion of maximum parsimony is not strictly how evolution work.
Especially in the case where a sequence undergoing evolution is also experiencing some kind of selection or maybe a biased mutation process.
E.g. in parsimony all substitutions have equal weight but in a sequence with rate heterogeneity some sites might evolve much faster than other.
Such complex processes are not accounted for in the MP method making it prone to errors \cite{felsenstein1978cases}, and since there is no consistent way of adding parameters to model a complex process, most researchers have turned to model based tree inference.
...
For the MP algorithm these ancestral states are part of the tree inference and found via. Fitch's algorithm, but for a model based method ancestral sequences must be estimated by extracting the maximum likelihood estimate of each internal node.
...
Especially after a clonal expansion many B cell will have the same genotype, and furthermore ancestral B cells might have the same genotype as their descendents.
...
In a GCtree B cells with the same genotype are merge into a single node carrying their total abundance and a leaf descending from a B cell with the same genotype is merged upwards, see figure \ref{fig:GCtree_illu}.
