...
Since the first vaccine research, by Edward Jenner on small pox, it has been clear that the human immune system is adaptable and able to protect its host from serious infections.
...
Despite the initial enthusiasm around Rep-Seq, the dust have settled, revealing the slightly disappointing reality.
...
Missing functional information for Rep-Seq data is what makes it difficult to interpret because there is no known truth to use as a reference.
...
% \subsection{Biology of B cells}
\section{Adaptive immunity}
The function of the immune system is to protect its host against invading pathogens.
To carry out this role the effector cells of the immune system have been equipt with a license to kill, and not only to kill pathogen, but also to kill its own host cells.
...
%Without more information one could think that the adaptive immunity is sufficient for protection and the innate immunity is superfluous when having a specific response from the adaptive immunity.
%This turns out to be wrong, and in fact innate immunity is much more important, and the reason is that there would not be any adaptive immunity without the help of the innate immunity.
...
% However the adaptive immune system always needs to be activated by the innate system before any event can happen here.

%To better understand this lets walk through an example of a simple bacterial infection cause by a skin tear.
...
At the same time a large number of B cell with different BCRs are monitoring the blood stream binding a variety of different protein.
Whatever these B cells bind to their BCR is getting transported into the cell, chopped up and presented in MHCII is a similar fashion as the macrophages.
...
When a TCR bind an MHCII:peptide complex presented by a B cell this activates the B cell to undergo a number of steps, which will be discussed in details later in this section.
...
% Antibodies are just a secreted from of the B cells BCRs.
%Clearance can occur rapidly and only when pathogens are flagged for phagocytosis by an antibody.
...
\begin{itemize}
  ...
  \item A T cell clone have TCRs binding the MHCII:peptide complex and gets clonally expanded.
...
%  \item The TCR of the clonally expanded T cells bind complementary MHCII:peptide complexes on a B cell and induces it to proliferate and produce antibodies.
\end{itemize}
...
Both heavy -and light chain starts with a variable region, defining their antigen binding capacity, and ends with a  a non-variable constant region (CH1-3/CL).
%Both heavy -and light chains have a non-variable constant region (CH1-3/CL), but at the be of each chain they have a variable region which defines its antigen binding.
...
%The rest of the observed diversity comes from another stochastic process involving trimming and inserting new nucleotides at the DNA ends flanking the D gene, see figure \ref{fig:VDJ_recomb}.
...
Furthermore all the junctional diversity is contained in the complementary defining region (CDR), as opposed to the framework region (FR), see figure \ref{fig:VDJ_recomb}.
...
%Light chain germline genes are present in two non-identical copies named kappa and lambda, residing on different chromosomes due to a duplication event.
...
Each gene is encoded by a single sequence, the allele, that can very between individuals.
% A gene can have many different observed alleles in a population, and although indels are possible, allelic difference between individuals usually alleles just differ by a single nucleotide polymorphism.
A gene can have many different observed alleles in a population, and usually allelic differences between individuals are just single nucleotide polymorphism, although indels are possible they are rare.
...
%All B -and T cell receptors are in the same protein structural family, the beta-sheet sandwich known as the immunoglobulin domain.
...
% However IMGT numbering was initially not structurally consistent for CDR insertions \cite{martin2010protein}.
...
Two things are required in the initial phase, a) T cells with a TCR specific for an MHCII-peptide from the immunizing antigen and b) B cells with a BCR that can bind this antigen, engulf it and present its peptides in MHCII for the T cells to bind, see figure \ref{fig:GC_reaction}.
...
The B cells are undergoing a mutation/selection process with two elements a) T cell help and selection and followed by b) mutation and proliferation.
...
In the LZ B cells are rolling along the large surface of the FDC membrane while the FDC is exposing full-sized, intact antigens for the BCRs to bind.
Some B cells will have BCR affinities strong enough that they are able to release antigen from the FDC membrane.
...
If sufficient Tfh binding is achieved the B cell is signaled to migrate to the DZ where it expresses high levels of the somatic hypermutation (SHM) inducing enzyme activation-induced cytidine deaminase (AID).
...
B Cells are constantly cycling between LZ and DZ, in a process called cyclic re-entry, until the GC is dissolved.
...
\begin{figure}
    ...
    \caption{
        ...
        Figure credit Frederick Matsen.
    }
\end{figure}
...
% GCs start out as highly polyclonal but competition and sporadic clonal bursts polarizes GCs during maturation and the majority ends up being dominated by cells with a single common ancestor \cite{tas2016visualizing}.
...
%Memory cells are preferentially secreted in the early maturation, are generally lower affinity and maintain a high degree of promiscuity \cite{Shinnakasu_2016}.
%Plasma cells secreted are all highly specific, high affinity clones \cite{victora2012germinal}.

%initiation by either naive, or memory B cells \cite{McHeyzer_2015}.
%Re-entry of a memory B cell into a GC can lead to accumulation of SHMs with phylogenetic manifestations.
%It is hypothesized that GC re-entry of an already mutated memory B cell will yield a distinct clonal family phylogeny with a long tree trunk going from the naive germline sequence to the most recent common ancestor (MRCA) from which all the sequences in the clonal family will be in short distance from \cite{yaari2015mutation}
...
\begin{figure}
    ...
    \caption{
        \label{fig:SAMM_plot}
        Plot of a 5'mer SHM motif model similar to S5F \cite{cui2016model}. Bottom row is the 5'-end of the motif progressing upwards to the 3'-end with G as the central base. Theta is the rate parameter of the survival model, used to fit the data, and is proportional to the logarithm of the mutation rate.
...
Latest development is to use micro-droplets to capture single cells and then perform the DNA prepping steps inside these drops.
...
% Now we begin to understand that the root cause of the clustering problem is propagation of point estimates, i.e. germline gene assignment to a BCR sequence that has undergone SHM and is associated with substantial uncertainty.
...
%%%% Naaah to so true because most samples are from a single individual with just one allele per gene.
%But while this probably is the main shortcoming of VJ junction agglomeration, ironically, because the method disregards the allele it is also prone to clustering of sequences from different germline alleles and thereby underestimating the number of clusters.
% Because it is too obvious that clustering on allele level is going to break due to mis-annotations, the allele level information a sequence might have is discarded and only the gene annotation is used, thereby increasing the chance of merging sequences with different germline alleles.
...
Motivated by Occam's Razor the best tree is simply the tree that with the fewest changed explain the observed data.
Practically this is done by calculating the tree score with Fitch's algorithm \cite{fitch1971toward} and then finding the tree with the lowest score.
Using the assumption of independence between sites ($p_i$) the parsimony score of a tree ($T$) can be calculated as a sum of all the score of each site: $S(T) = \sum_{i=1}^m F(p_i | T)$.
With a score parsimony function the possible tree space can be searched and the maximum parsimony (MP) tree can be found.
...
Instead various heuristic tree moves like, nearest neighbor interchange (NNI), subtree pruning and regrafting (SPR) and tree bisection and reconnection (TBR) are used to explore widely around the tree space.

It it clear that the MP method for tree inference is very simple and while this is good in some applications the criterion of maximum parsimony is not necessarily how evolution work.
Especially in the case where the sequence undergoing evolution is also experiencing some kind of selection or maybe a bias mutation process.
Complex processes are generally not accounted for in MP method and since there is not consistent way of adding parameters to address a complex process most have turned to model based tree inference.



\subsection{Model based tree inference}
As an alternative to maximizing the parsimony if one could formulate a likelihood function for the data given the tree, then the problem turn into finding the most likely tree, referred to as maximum likelihood (ML).
The likelihood function calculating the likelihood of the data given a tree with branch length and a model for substitutions in the sequence.
Branch lengths are now a distances in a continuous mutation process driven by a mutation rate.
To accommodate more advanced things such as differences in substitution rates, e.g. higher rate of \texttt{T->G} then \texttt{T->A}, a rate matrix can be defined.
If the data is at DNA level the rates are going to be rates of substitution between all the DNA characters e.g the F81 model \cite{felsenstein1981evolutionary}, or the widely used general time reversible (GTR) model \cite{tavare1986some}.

DNA substitution models are not aware of protein coding and therefore does not explicitly discriminate between synonymous and non-synonymous mutations, something that is otherwise crucial for protein under tight selection.
...
There are information lost in the translation process so this needs to be outweighed by the gain in performance if an amino acid model is to replace the DNA model.
Alternatively a codon model is defined on DNA level with DNA level substitutions working on units defined by codons.
In that way a codon model will both account for difference in the rate of synonymous and non-synonymous substitutions and in the rate difference between amino acids.
Two popular choices for a codon model exists: GY94 \cite{goldman1994codon}, MG94 \cite{muse1994likelihood}.



\subsubsection{Bayesian phylogenetics}
In the ML framework tree, branch length and substitution parameters are all point estimates adjusted to maximize the likelihood function but this might not always be desirable.
E.g. when information like time or rate is to be extracted from a tree a single point estimate is not very useful if the variance of such estimate is unknown.
Bayesian phylogeny address this challenge by integrating over the distribution of trees, branch lengths and substitution parameters by Monte Carlo sampling.
When sampling has converged the resulting posterior distributions of all parameters can be used to estimate confidence in the maximum a posteriori (MAP) estimate e.g. by providing the high density interval (HDI) along with the MAP estimate.
...
\iffalse
\subsection{Phylogeny of a clonal family}
In this work we consider how to apply phylogeny to BCR sequence evolution.
Specifically we used the definition of a clonal family that has single shared root state given a priori as a naive sequence.
Often clonal families and their naive sequences are inferred from high throughput sequencing (HTS) data generated from bulk RNA extracted from peripheral blood mononuclear cell (PBMC).
However PBMCs do not contain a single time point of B cell from the GC reaction, rather they contain memory -and plasma cells that have been continuously exported while their GC reaction was taking place.


%%% The problem gets harder with PBMC samples because this may be containing multiple sample times because the memory/plasma cells fraction has been exported throughout the GC reaction.

% Standard phylogeny does not handle cases with multiple time point samples, or sampling of ancestral states in general.
\fi
...
As a side result of inferring a phylogeny it is also possible to reconstruct the sequence of those tree node that are internal and unobserved.
This process is known as ancestral sequence reconstruction (ASR).
For the MP these ancestral states are part of the tree and found via. Fitch's algorithm, but for a model based method the sequence must be estimated by extracting the maximum likelihood estimate of each internal node.
ASR on ML trees can either be done as a marginal -or a joint reconstruction.
In the marginal case each node is reconstructed independently from one another and each is the ML estimate without fixing the sequence of any of the other internal nodes.
Notice that when one ancestral sequence is found this will change likelihood function for the tree and thereby effect all the other internal node.
This effect is ignored in marginal reconstruction but addressed in joint reconstruction where the problem is to find the ML estimate of all the sequences at the same time.

ASR is an important way of exploring the evolutionary trajectory and ancient proteins.
It has been used to investigate the specificity of the important cancer drug type kinase inhibitor \cite{Wilson2015-vi} and studying evolution of broadly neutralizing HIV antibodies \cite{Doria-Rose2014-vi}.
Yet there have only been sparse validation of ASR methods partly because it is difficult to test experimentally.
A clever study by Randall et al. \cite{randall2016experimental} made an artificial evolution and by both sampling ancestors and a final time point they could experimentally validate ASR inference methods which showed high accurary of 97.17-98.88\%.



\subsection{Genotype collapsed tree}
While B cell in a GC reaction are having a high mutation rate there are still many B cell with the same genotype.
Especially after a clonal expansion many B cell will have the same genotype, furthermore the ancestral B cell might have the genotype as its descendents.
BCR sequence are in general low complexity but because there are many B cells in a GC their phylogenetic trees are complex with hundreds to thousands of taxa.
To reduce tree complexity we use the concept of a genotype collapsed tree (GCtree).
In the GCtree B cells with the same genotype are merge into a single node carrying their total abundance and a leaf descending from a B cell with the same genotype is merged upwards, see figure \ref{fig:GCtree_illu}.
...
We found that the GCtree gives a good visualization of the BCR phylogeny enabling interpretations such as recent clonal bursts, fitness advantages etc.
...
Figure \ref{fig:collapsed_tree_example} is an example of how a tree is collapsed and represented in GCtree format.
Embedded in the GCtree visualization are a number of traits: genotype abundance is annotated in the middle of each node and proportional to the node size, dashed branches represents synonymous mutations while fully drawn lines are non-synonymous, branch length is proportional to the hamming distance between nodes and branch thickness is proportional to the number of amino acid differences.
...
\begin{figure}
    ...
    \caption{
        ...
        In a) a full lineage tree showing a simulation over several rounds of replication.
        Only the leafs are sampled at the end of the simulation.
        b) GCtree version of the tree in a).
        The GCtree only shows counts for the leafs on its source tree and node size corresponds to abundance.
        In the GCtree synonymous mutations are indicated by dash branch lines, branch length is reflecting the hamming distance at DNA level and solid branch line thickness reflects the number of non-synonymous mutations.
    }
\end{figure}
...
ImmuneDB \cite{rosenfeld2017immunedb} (alternative to CFT)


Combining clustering with phylogeny, basically CFT.
Add an arbitrary amount of extra meta information.
